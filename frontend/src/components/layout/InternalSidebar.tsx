...
export function InternalSidebar({ userEmail }: { userEmail: string }) {
  ...
  return (
    <>
      ...
      {/* Sidebar - mobile */}
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 z-50 w-64 bg-slate-800 transform transition-transform duration-200 ${
      ...
      {/* Sidebar - desktop */}
      <aside className="hidden lg:block fixed inset-y-0 left-0 w-64 bg-slate-800">
        {sidebarContent}
      </aside>
    </>
  );
