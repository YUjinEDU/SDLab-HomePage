...
- [ ] **SEC-01**: `lib/permissions/index.ts` 구현 — `assertRole()` (Server Actions용), `requireRole()` (레이아웃용) 유틸 export
- [ ] **SEC-02**: 모든 professor/admin Server Actions(`publications`, `projects`, `members`, `news`, `patents`)에 `assertRole` 역할 검사 추가
