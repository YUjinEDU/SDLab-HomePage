...
> **Summary**: 스마트데이터연구실 공개 포털 MVP - 7개 페이지 + 공통 컴포넌트 상세 설계
>
...
```
frontend/src/data/
  members.ts        -> Member[] (교수님 실제 데이터 + 샘플 3명)
  research-areas.ts -> ResearchArea[] (3대 분야)
  publications.ts   -> Publication[] (샘플 5편)
  patents.ts        -> Publication[] (type="patent", 샘플 3건)
  projects.ts       -> Project[] (샘플 4개)
  news.ts           -> NewsItem[] (샘플 4개)
...
│  SiteHeader                                      │
│  ┌─────────────────────────────────────────────┐│
│  │ Logo  Nav(Members Research Projects       ││
│  │       Publications Patents Contact)  CTA ││
│  └─────────────────────────────────────────────┘│
├─────────────────────────────────────────────────┤
│                                                  │
...
│ [PageHero] Patents                               │
├─────────────────────────────────────────────────┤
│ [StatsPanel] 전체 N건 | 연도 범위                 │
├─────────────────────────────────────────────────┤
│ [SearchInput] 특허명, 발명자, 키워드 검색          │
│ [YearFilter] 연도 선택                            │
├─────────────────────────────────────────────────┤
│ [PatentList] - card list                         │
│  ┌─────────────────────────────────────────┐    │
│  │ [특허배지] 제목                           │    │
│  │ 발명자 목록 (멤버 링크)                   │    │
│  │ 특허청 · 연도                             │    │
...
| `MainNavigation`         | `components/navigation/MainNavigation.tsx`         | -                      | 데스크톱 GNB (개별 드롭다운, Members만 서브링크) |
| `MobileNavigationDrawer` | `components/navigation/MobileNavigationDrawer.tsx` | `isOpen, onClose`      | 모바일 아코디언 메뉴 (Members 서브링크)          |
...
#### Patents (`components/patents/`)

| Component    | Props                 | Client? |
| ------------ | --------------------- | ------- |
| `PatentCard` | `patent: Publication` | No      |
...
frontend/src/app/
  (public)/
    ...
    patents/
      page.tsx          -> PatentsPage (필터/검색)
      [slug]/
        page.tsx        -> PatentDetailPage (크로스 링크 포함)
    contact/
...
| Patents            | `data/patents.ts`                                                                                                                           | Server + Client (filter/search) |
| Patent Detail      | `data/patents.ts`, `data/members.ts`, `data/projects.ts`, `data/research-areas.ts`                                                          | Server (크로스 링크)            |
...
5. [x] components/navigation/MainNavigation.tsx (개별 드롭다운, Members만 서브링크)
6. [x] components/navigation/MobileNavigationDrawer.tsx (아코디언 메뉴, Members 서브링크)
...
### 9.10 Phase 10: Patents Page (신규)

```
Priority: MEDIUM
Estimated files: ~4

1. [x] data/patents.ts (Publication[] type="patent", 샘플 3건)
2. [x] components/patents/PatentCard.tsx
3. [x] app/(public)/patents/page.tsx (필터/검색/페이지네이션)
4. [x] app/(public)/patents/[slug]/page.tsx (크로스 링크 포함)
```

### Total Estimated Files: ~67
...
| 0.3     | 2026-03-08 | Patents page added (목록 + 상세), navigation restructured (mega menu → 개별 드롭다운, Research/Projects 독립 메뉴), data/patents.ts 추가, components/patents/ 추가, 7 pages total                                                                                                     | Claude Code |
